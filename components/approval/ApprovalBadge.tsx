...
const config: Record<ApprovalStatus, { label: string; color: string; bg: string; border: string }> = {
  draft:     { label: "임시저장", color: "var(--text-muted)",    bg: "var(--surface-sub)", border: "var(--border)" },
  ...
  cancelled: { label: "취소됨",   color: "var(--text-muted)",    bg: "var(--surface-sub)", border: "var(--border)" },
};
